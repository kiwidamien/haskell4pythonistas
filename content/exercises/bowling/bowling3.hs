...
type Roll = Int
type IndexedRoll = (Int, Roll)

data ClosedFrame = NormalFrame Roll Roll | StrikeFrame | SpareFrame Int Int 
data Frame = OpenFrame Int | ClosedFrame
...
rollOntoStack :: [Frame] -> Roll -> [Frame]
rollOntoStack [] 10 = [StrikeFrame]
rollOntoStack [] roll = [OpenFrame roll]
rollOntoStack fs roll = go fs (last fs) roll
  where go fs ClosedFrame 10 = fs ++ [StrikeFrame]
        go fs ClosedFrame r  = fs ++ [OpenFrame r]
        go fs (OpenFrame r1) r2
          | r1 + r2 == 10 = (init fs) ++ [StrikeFrame r1 r2]
          | r1 + r2 > 10 = error "Not allowed!"
          | otherwise = (init fs) ++ [NormalFrame r1 r2]
